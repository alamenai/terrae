import { Zap, Moon, Puzzle, Code, SquaresIntersect } from "lucide-react"
...
const features = [
  {
    ...
    description: "Works with both Mapbox GL JS and MapLibre GL JS.",
  },
  {
    icon: Puzzle,
...
export default function IntroductionPage() {
  return (
    ...
    >
      <DocsSection>
        <p>
          <strong className="text-foreground">Terrae</strong>{" "}
          <span className="italic text-muted-foreground">(pronounced TER-ray)</span> provides beautifully designed,
          animated, accessible, and customizable map components. Built on{" "}
      ...
      <DocsSection title="Why Terrae?">
        <p>
          Building interactive maps in React has always been frustrating. Existing solutions either require too much
          boilerplate, follow{" "}
          <DocsLink href="https://docs.mapbox.com/mapbox-gl-js/guides/install/" external>
            imperative patterns
          </DocsLink>{" "}
          that don&apos;t align with React&apos;s declarative philosophy, or lack integration with modern tooling like
          shadcn/ui. Terrae was created to solve this—providing composable, beautiful map components that feel natural
      ...
      <DocsSection title="Features">
        <div className="grid gap-6 sm:grid-cols-2 mt-6 not-prose">
          {features.map((feature) => (
            <div key={feature.title} className="rounded-3xl border bg-card p-6 space-y-4">
      ...
      <DocsSection title="Install What You Need">
        ...
        <CodeBlock
          code={`npx shadcn@latest add https://www.terrae.dev/map.json
npx shadcn@latest add https://www.terrae.dev/marker.json
npx shadcn@latest add https://www.terrae.dev/controls.json`}
          language="bash"
        />
        <p className="mt-4">
          <DocsLink href="/docs/components">View all available components →</DocsLink>
        </p>
      </DocsSection>
    </DocsLayout>
  )
