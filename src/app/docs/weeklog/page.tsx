...
import { UpcomingWeekendAnimation } from "@/components/upcoming-weekend-animation"

export const metadata: Metadata = {
  title: "Weeklog",
  description:
    "Follow along with Terrae's weekend development journey. New features and improvements crafted during weekend coding sessions.",
}
...
type WeeklogEntry = {
  ...
  date: string
  upcoming?: boolean
  components?: WeeklogFeature[]
  properties?: WeeklogFeature[]
  fixes?: WeeklogFeature[]
}
...
type Weeklog = WeeklogEntry | AnnouncementEntry

type FeatureSectionProps = {
  title: string
  items: WeeklogFeature[]
...
export default function WeeklogPage() {
  return (
    <>
      <div className="mb-12">
        <h1 className="text-4xl font-bold tracking-tight mb-4">Weeklog</h1>
        <p className="text-muted-foreground text-lg leading-relaxed max-w-3xl">
          Welcome to the Terrae Weeklog. Every weekend, I dedicate time to building and improving this library. Here
          you'll find detailed updates on new features, improvements, and fixes shipped during each weekend coding
          session.
        </p>
      </div>
      ...
      <div className="space-y-12">
        {weeklogs.map((weeklog, index) => {
          ...
          const entry = weeklog as WeeklogEntry

          if (entry.upcoming) {
            return (
              <div
                key={index}
                id={`week-${entry.weekend}`}
          ...
          return (
            <div
              key={index}
              id={`week-${entry.weekend}`}
              className="rounded-3xl border border-border/50 p-8 sm:p-12 space-y-8"
            >
              <div className="space-y-2">
                <h2 className="text-3xl font-bold">Week {entry.weekend}</h2>
                <p className="text-muted-foreground">{entry.date}</p>
              </div>
