...
const COVID_HOTSPOTS: CovidHotspot[] = [
...
const COVID_ORIGIN: [number, number] = [114.3055, 30.5928]

const COVID_SPREAD_DESTINATIONS: [number, number][] = [
  [-95.7129, 37.0902],
  [-51.9253, -14.235],
  [78.9629, 20.5937],
  [-3.436, 55.3781],
...
const getPulseColor = (severity: CovidHotspot["severity"]) => {
  if (severity === "high") {
...
const getPulseSize = (severity: CovidHotspot["severity"]) => {
  if (severity === "high") {
    return 100
  }
  if (severity === "medium") {
...
function CovidHotspotsExample() {
  return (
    <div className="w-full h-full relative">
      <InfoPanel title="Active Hotspots">
        <div className="text-2xl font-semibold leading-tight">{COVID_HOTSPOTS.length}</div>
        <div className="flex items-center gap-1 mt-1">
    ...
    <div className="w-full h-full relative">
      <InfoPanel title="Global Spread">
        ...
        <div className="flex items-center gap-1 mt-1">
          ...
          <span className="text-xs text-muted-foreground">{COVID_SPREAD_DESTINATIONS.length} regions</span>
        </div>
      </InfoPanel>
      ...
      >
        <MapLineRadial
          id="covid-spread"
          origin={COVID_ORIGIN}
          destinations={COVID_SPREAD_DESTINATIONS}
          color="#f43f5e"
          width={2}
          curvature="auto"
          duration={2000}
...
export function Examples() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
      ...
      {/* Widget 5: COVID-19 Hotspots with Animated Pulse */}
      <ExampleCard label="COVID-19 Hotspots" className="h-80 sm:h-96 lg:h-[28rem]">
        <CovidHotspotsExample />
      </ExampleCard>

      {/* Widget 6: COVID-19 Spread Animation */}
      <ExampleCard label="COVID-19 Spread" className="h-80 sm:h-96 lg:h-[28rem]">
        <CovidSpreadExample />
      </ExampleCard>

      {/* Widget 7: Animated Route */}
