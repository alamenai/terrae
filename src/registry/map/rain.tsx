 ...
 */
export function createZoomInterpolation(
  value: number,
  minZoom: number = 11,
  maxZoom: number = 13
): any[] {
...
export function MapRain({
  ...
  useEffect(() => {
    ...
    // Check if map.setRain is available (requires Mapbox GL JS v3.9+)
    if (typeof (map as any).setRain !== "function") {
      console.warn(
        "Rain effect requires Mapbox GL JS v3.9 or higher. Please update your mapbox-gl version."
      );
      return;
    }
